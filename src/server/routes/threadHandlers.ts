...
import type { InsightThread, Note, NoteChunk } from '../types';
import { getInsightThreadsCache, upsertInsightThreadsCache } from '../database';
import {
  prepareNotes,
  cluster,
  clusterLPA,
...
import { envNum, envBool01, MAX_NOTES } from '../config';
...
async function runThreadGeneration(jobId: string, userId: string, token: string, excludeSingletons: boolean) {
  ...
  try {
    ...
    for (const cluster of filtered) {
      ...
      const { title, summary } = await summarizeThread(noteObjs as any);
      threads.push({
...
export async function handleGetThreads(req: any, res: any) {
...
export async function handleGenerateThread(req: any, res: any) {
  const auth = await requireUser(req, res);
  if (!auth) return;
