...
import type { Chunk, AskRequest, AskResult } from "../lib/rag/types";

declare const self: DedicatedWorkerGlobalScope;

let cache: { chunks: Chunk[]; bm25: ReturnType<typeof buildBM25> | null } = { chunks: [], bm25: null };

async function ensureChunks(notes?: any[]) {
  if (cache.chunks.length > 0) return cache.chunks;
  let allNotes: any[] = notes || [];
  try {
    if (!allNotes.length) {
...
self.addEventListener("message", async (e: MessageEvent) => {
  ...
  try {
    if (type === "ask") {
      const p = payload as AskRequest;
      const chunks = await ensureChunks(p.notes);
      if (chunks.length === 0) { self.postMessage({ id, ok:true, result: { keypoints: [], citations: [] } }); return; }
      const [qvec] = await embed(p.engine, [p.q]);
      // Precompute chunk vecs in batches
      const texts = chunks.map(c => c.text);
      const vecs: number[][] = new Array(chunks.length);
      for (let i=0;i<texts.length;i+=64){
        const part = texts.slice(i, i+64);
        const emb = await embed(p.engine, part);
        for (let j=0;j<emb.length;j++) vecs[i+j] = emb[j];
      }
      ...
      const seen = new Set<string>(); const keypoints:string[] = []; const citations:any[] = [];
      for (const it of picked){
        const c = chunks.find(c => c.id === it.id);
        if (!c) continue;
        if (!seen.has(c.noteId)){ keypoints.push(c.text); seen.add(c.noteId); }
        citations.push({ text: c.text, noteId: c.noteId, pos: c.pos, tags: c.tags, createdAt: c.createdAt });
        if (keypoints.length>=3) break;
      }
      self.postMessage({ id, ok: true, result: { keypoints, citations } as AskResult });
      return;
    }
    if (type === "dedup") {
      const p = payload || {};
      const notes = (p.notes || []) as Array<{id:string; content:string; createdAt:number; tags:string[]}>;
      const thr: number = typeof p.threshold === "number" ? p.threshold : 0.92;
      ...
      for (let i=0;i<maxN;i++){
        for (let j=i+1;j<maxN;j++){
          ...
          if (s >= thr) pairs.push({ a:i, b:j, s });
        }
      }
      const parent = new Array(maxN).fill(0).map((_,i)=>i);
      const find=(x:number): number => parent[x]===x? x: (parent[x]=find(parent[x]));
      const uni=(x:number,y:number)=>{ x=find(x); y=find(y); if(x!==y) parent[y]=x; };
      for (const p2 of pairs) uni(p2.a, p2.b);
      const groups = new Map<number, number[]>();
      for (let i=0;i<maxN;i++){ const r=find(i); if(!groups.has(r)) groups.set(r,[]); groups.get(r)!.push(i); }
      const result = Array.from(groups.values()).filter(g=>g.length>1).map(g=> ({ ids: g.map(i=> notes[i].id), score: 1, size: g.length }));
      self.postMessage({ id, ok:true, result });
      return;
    }
    if (type === "resetIndex") { cache = { chunks: [], bm25: null }; self.postMessage({ id, ok:true, result:"ok" }); return; }
    self.postMessage({ id, ok:false, error:"unknown command" });
  } catch (err:any) { self.postMessage({ id, ok:false, error: String(err?.message || err) }); }
});
