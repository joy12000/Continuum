...
const CalendarPage: React.FC = () => {
  ...
  const onPrev=()=>{ const d=new Date(y,m,1); d.setMonth(d.getMonth()-1); setY(d.getFullYear()); setM(d.getMonth()); };
  const onNext=()=>{ const d=new Date(y,m,1); d.setMonth(d.getMonth()+1); setY(d.getFullYear()); setM(d.getMonth()); };
  const onToday=()=>{ const d=new Date(); setY(d.getFullYear()); setM(d.getMonth()); setSelDate(ymd(d)); };

  const monthTitle = `${K_MONTH[m]} ${y}`;
  const dayNotes = mapByDate[selDate] || [];

  const pasteToSky = (t:string)=>{ window.dispatchEvent(new CustomEvent("sky:paste",{ detail:{ text:t } })); nav("/"); };

  return (
    <div className="calendar-wrap content-offset">
      <header className="cal-head">
        <button className="nav-btn" onClick={()=>nav("/")}>홈</button>
        <div className="cal-title">
          <button aria-label="이전 달" onClick={onPrev}>‹</button>
          <h1>{monthTitle}</h1>
          <button aria-label="다음 달" onClick={onNext}>›</button>
        </div>
        <div className="cal-actions"><button onClick={onToday}>오늘</button></div>
      </header>
      ...
      <section className="cal-detail" aria-live="polite">
        ...
        <div className="detail-list">
          ...
          {!loading && dayNotes.length===0 && <div className="empty">이 날의 노트가 없어요</div>}
          {dayNotes.map((n)=>(
            <article key={String(n.id)+n.createdAt} className="detail-item">
              ...
              <div className="detail-actions">
                <button onClick={()=>pasteToSky(n.content)}>밤하늘에 불러오기</button>
