...
const SearchResultsList: React.FC<SearchResultsListProps> = ({ results, loading, noteTitlesMap, query, onNoteClick }) => {
  ...
  const handleFeedback = (result: SearchResult, newFeedback: 'like' | 'dislike') => {
    const key = `${result.document_id || result.note_id || 'unknown'}_${result.chunk_index ?? result.uri ?? result.content.slice(0, 20)}`;
    setFeedback(prev => ({
      ...prev,
      [key]: prev[key] === newFeedback ? null : newFeedback
    }));
    // Here you would typically store this feedback, e.g., in your database
    console.log(`Feedback for note ${result.note_id || result.document_id || 'unknown'}: ${newFeedback}`);
  };

  return (
    <ul className="space-y-4">
      {results.map((result, idx) => {
        const key = `${result.document_id || result.note_id || 'unknown'}_${result.chunk_index ?? idx}`;
          ...
          <li key={key}>
            <div className="block w-full text-left border border-slate-700/50 bg-slate-900/60 backdrop-blur-lg rounded-3xl p-4 transition-all duration-300 hover:bg-slate-800/70 hover:shadow-xl">
              <div className="flex justify-between items-start gap-4">
                <button onClick={() => result.note_id && onNoteClick(result.note_id)} className="flex-grow text-left" disabled={!result.note_id}>
                  <h3 className="text-lg font-semibold text-primary hover:underline">{noteTitlesMap[result.document_id || ''] || (result.note_id && noteTitlesMap[result.note_id]) || '제목 없는 노트'}</h3>
              ...
              <div className="text-xs text-muted-foreground/80 mt-3 pt-2 border-t border-slate-800">
                ...
                <span>점수: {(result.similarity ?? result.score ?? 0).toFixed(3)}</span>
              </div>
            </div>
          </li>
        );
