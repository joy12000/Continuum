...
const CalendarMonth: React.FC<Props> = ({ year, month, weekLabels, notesByDate, selectedDate, onSelectDate, isCompact }) => {
  const cells = useMemo(() => {
    ...
    if (isCompact) {
      const selectedIndex = arr.findIndex(c => c.key === selectedDate);
      if (selectedIndex !== -1) {
        const weekStart = Math.floor(selectedIndex / 7) * 7;
        return arr.slice(weekStart, weekStart + 7);
      } else {
    ...
    return arr;
  }, [year, month, selectedDate, isCompact]);

  const todayKey = ymd(new Date());

  return (
    <div className={`cal-grid rounded-3xl transition-all duration-300 ease-in-out`}>
      <div className="cal-weekhead">
        {weekLabels.map((w, i) => (
          <div key={i} className={`wcell ${i === 0 ? "sun" : ""} ${i === 6 ? "sat" : ""}`}>{w}</div>
        ))}
      </div>
      <div className="cal-cells">
        {cells.map((c) => {
          ...
          return (
            <button
              key={c.key}
              className={`ccell ${active ? "active" : ""} ${today ? "today" : ""} ${c.inMonth ? "" : "dim"}`}
              onClick={() => onSelectDate(c.key)}
              aria-current={active ? "date" : undefined}
              aria-label={`${year}년 ${month + 1}월 ${c.date}일, 노트 ${list.length}개`}
            >
