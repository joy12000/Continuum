...
export default function App() {
    ...
    const renderView = () => {
        switch (view) {
            case 'settings':
                return _jsx(Settings, { onNavigateToDiagnostics: () => setView('diagnostics') });
            case 'diagnostics':
                return _jsx(Diagnostics, { onBack: () => setView('settings') });
            case 'today':
            default:
                return (_jsx(TodayCanvasScreen, { onNavigate: setView, query: q, onQueryChange: setQ, notes: finalResults, onSearchFocus: handleSearchFocus, suggestedQuestions: suggestedQuestions, isLoadingSuggestions: isLoadingSuggestions, suggestionError: suggestionError, generatedAnswer: generatedAnswer, onNewNote: handleNewNote }));
        }
    };
    return (_jsxs("div", { children: [_jsx(Toasts, {}), renderView()] }));
}
