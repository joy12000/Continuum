...
export function useGeneratedAnswer() {
  ...
  async function generateSummaryAfterSave(noteText: string, userId: string) {
    try {
      ...
      const similarChunks = (similarResults?.results || []) as Array<{ note_id?: string | null; similarity?: number | null; score?: number | null }>;

      // 2. Get top 3 unique note IDs from chunks
      const uniqueNoteIds = [...new Set(similarChunks
        .filter((c: any) => (c.similarity ?? c.score ?? 0) >= 0.3)
        .map((c: any) => c.note_id)
