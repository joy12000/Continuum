...
export async function deriveAesKeyFromPassword(password, salt) {
    ...
    return crypto.subtle.deriveKey({ name: "PBKDF2", salt, iterations: 100_000, hash: "SHA-256" }, keyMaterial, { name: "AES-GCM", length: 256 }, true, // 키를 내보낼 수 있도록 설정 (필요 시)
    ["encrypt", "decrypt"]);
}
/**
 ...
 */
export async function decryptJSON(file, password) {
