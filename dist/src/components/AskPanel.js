...
export function AskPanel({ engine, setQuery, notes }) {
    ...
    return (_jsxs("div", { className: "card bg-base-100 shadow-xl mt-4", children: [_jsxs("div", { className: "card-body", children: [_jsxs("div", { className: "flex space-x-2", children: [_jsx("input", { ref: qRef, className: "input input-bordered w-full", placeholder: "\uC9C8\uBB38\uC744 \uC785\uB825\uD558\uC138\uC694...", onKeyDown: (e) => {
                                    if (e.key === "Enter")
                                        onAsk();
                                } }), _jsx("button", { className: "btn btn-primary", onClick: onAsk, disabled: isGenerating, children: isGenerating ? _jsx("span", { className: "loading loading-spinner" }) : "질문하기" })] }), _jsxs("div", { className: "flex items-center space-x-4 text-sm mt-2", children: [_jsxs("label", { className: "label cursor-pointer flex-1", children: [_jsxs("span", { className: "label-text", children: ["\uC758\uBBF8\uB860\uC801 \uAC00\uC911\uCE58: ", alpha.toFixed(2)] }), _jsx("input", { type: "range", min: 0, max: 1, step: 0.05, value: alpha, onChange: (e) => setAlpha(parseFloat(e.target.value)), className: "range range-xs" })] }), _jsxs("label", { className: "label cursor-pointer flex-1", children: [_jsxs("span", { className: "label-text", children: ["MMR \u03BB: ", lambda.toFixed(2)] }), _jsx("input", { type: "range", min: 0, max: 1, step: 0.05, value: lambda, onChange: (e) => setLambda(parseFloat(e.target.value)), className: "range range-xs" })] })] }), showGenAnswer && resGen ? (_jsx(GeneratedAnswer, { data: resGen, noteTitlesMap: {}, onNoteClick: handleNoteClick })) : showGenAnswer && isGenerating ? (_jsx("div", { className: "text-center text-slate-500 animate-pulse", children: "AI\uAC00 \uB2F5\uBCC0\uC744 \uC0DD\uC131 \uC911\uC785\uB2C8\uB2E4..." })) : showGenAnswer && genError ? (_jsx("div", { className: "text-center text-red-500", children: genError.message })) : (res && _jsx(AnswerCard, { kp: res.kp, cites: res.cites, onJump: (id) => setQuery("#" + id) }))] }), selectedNoteId && _jsx(NoteDetailModal, { noteId: selectedNoteId, isOpen: isModalOpen, onClose: () => setIsModalOpen(false) })] }));
}
