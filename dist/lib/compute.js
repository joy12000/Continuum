...
// 내부용 인접리스트
function _buildAdj(n, edges, minW = 0) {
    const adj = new Map();
    for (let i = 0; i < n; i++)
        adj.set(i, []);
    for (const e of edges) {
 ...
 * - 셔플 없이 결정적 순회
 */
export function clusterLPA(prepared, edges, opts) {
    const n = prepared.length;
...
// 임계값으로 DSU
function _clusterByThreshold(n, edges, thr) {
    const dsu = new DSU(n);
    for (const e of edges)
        if (e.score >= thr)
            dsu.u(e.i, e.j);
...
// 가중 모듈러리티
function _modularityWeighted(n, edges, clusters) {
 ...
 */
export function clusterByAutoThreshold(prepared, edges, opts) {
    const n = prepared.length;
    ...
    if (scores.length === 0)
        return { clusters: [Array.from({ length: n }, (_, i) => i)], threshold: 1, Q: 0 };
    const q = (p) => scores[Math.min(scores.length - 1, Math.max(0, Math.floor(p * (scores.length - 1))))];
...
export function detectIsolatedNodes(n, edges, isoCut = 0.08) {
    ...
    for (let i = 0; i < n; i++) {
        const w = maxW[i];
        if (!Number.isFinite(w) || w < isoCut)
            iso.add(i);
    }
    return iso;
}
// 간선 성기화: 상호 k-최근접 이웃(mutual kNN)
function _topKByNode(n, edges, k) {
...
export function sparsifyEdgesKNN(n, edges, k = 8, { mutual = true, minScore = 0.08 } = {}) {
    if (edges.length === 0)
        return edges;
...
// === 싱글톤 흡수(고립 제외): 과분할 완화 ===========================
function _absorbSingletons(clusters, edges, iso) {
    const out = [];
    const singles = [];
    for (const c of clusters) {
        if (c.length === 1 && !iso.has(c[0]))
            singles.push(c[0]);
        else
            out.push(c);
    }
    if (singles.length === 0 || out.length === 0)
        return [...out, ...singles.map(v => [v])];
    // 노드→이웃(가중치) 맵
    const adj = new Map();
    for (const e of edges) {
        if (!adj.has(e.i))
...
export function forceSplitByMST(n, edges, k) {
    if (k <= 1 || n === 0)
...
export function clusterHybrid(prepared, edgesAll, pref) {
    const n = prepared.length;
    ...
    const edges = sparsifyEdgesKNN(n, edgesDense, knnK, { mutual, minScore: Math.max(0.005, minEdge * 0.5) });
    // 2) LPA 1차
    ...
    if (clusters.length === 1 && nonIsoNodes.size >= 2) {
        ...
        for (const comp of comps)
            clusters.push(comp.map(p => rev[p]));
    }
    // 6) 싱글톤 흡수(고립 제외)로 과분할 완화
    clusters = _absorbSingletons(clusters, edges.length ? edges : edgesAll, iso);
    // 7) 비어있는 군집 제거
    clusters = clusters.filter(c => c.length > 0);
    return { clusters, method: "hybrid+knn+isolation+absorb", meta: { knnK, mutual, isoCut } };
}
