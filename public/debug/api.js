...
(function(){
  ...
  async function generate(){ set('POST /api/remote/generate ...'); try{ const payload = { question:'What is Continuum?', context:[{id:'d1', content:'Continuum is offline-first.'}] }; const {r,t} = await f('/api/remote/generate', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) }); log('Status '+r.status); log(t.slice(0,1200)); }catch(e){ log('ERR '+e); } }
